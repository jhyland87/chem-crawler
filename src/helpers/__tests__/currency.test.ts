import { getCurrencySymbol, getCurrencyRate, getCurrencyCodeFromSymbol, toUSD, parsePrice } from '../currency'
...
describe('getCurrencyRate', () => {
  ...
  it('should return exchange rate for valid currency pair', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('https://hexarate.paikama.co/api/rates/latest/USD?target=EUR');
  })

  it('should throw error for failed API call', async () => {
    (global.fetch as jest.Mock).mockRejectedValueOnce(new Error('API Error'))

    await expect(getCurrencyRate('USD', 'EUR')).rejects.toThrow('Failed to get currency rate for USD to EUR')
  })
})

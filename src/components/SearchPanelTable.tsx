...
import SearchPanelTableToolbar from "./SearchPanelTableToolbar";
...
export default function SearchPanelTable({
  ...
  useEffect(() => {
    if (!isEmpty(settingsContext.settings.hideColumns)) {
      table.getAllLeafColumns().map((column: Column<Product, unknown>) => {
        if (settingsContext.settings.hideColumns.includes(column.id))
          column.toggleVisibility(false);
      });
    }
    chrome.storage.local
      .get(["searchResults", "paginationModel"])
      .then((data) => {
        ...
        setSearchResults(
          Array.isArray(storedSearchResults) ? storedSearchResults : []
        );
        setStatusLabel("");
      });
  });
  ...
  async function executeSearch(query: string) {
    ...
    console.debug(
      `Found ${resultCount} products in ${searchTime} milliseconds`
    );
        ...
        if (!searchResults.length) {
          setStatusLabel(
            isLoading ?
              ...
              : "Type a product name and hit enter"
          );
          return;
        }

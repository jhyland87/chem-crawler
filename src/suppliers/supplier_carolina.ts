...
import { Product, HeaderObject, QuantityObject } from "../types";
  ...
  implements AsyncIterable<T>
{
  ...
  protected async queryProducts(): Promise<void> {
    ...
    const productElements: NodeListOf<HTMLElement> = doc.querySelectorAll(
      "div.c-feature-product"
    );
    ...
    for (const elem of productElements) {
      elementList.push({
        title: _trimSpaceLike(
          (elem.querySelector("h3.c-product-title") as HTMLElement)?.innerText
        ),
        url: _trimSpaceLike(
          (
            elem.querySelector("a.c-product-link") as HTMLAnchorElement
          )?.href?.replace(/chrome-extension:\/\/[a-z]+/, "")
        ),
        prices: _trimSpaceLike(
          (elem.querySelector("p.c-product-price") as HTMLElement)?.innerText
        ),
        count: _trimSpaceLike(
          (elem.querySelector("p.c-product-total") as HTMLElement)?.innerText
        ),
      });
    }
  ...
  protected async parseProducts(): Promise<any> {
    return Promise.all(
      this._queryResults.map((result) => this._getProductData(result))
    );
  ...
  protected async _getProductData(
    ...
    try {
      const response = await this.httpGet(
        `https://www.carolina.com${productIndexObject.url}`
      );
      if (!response?.ok) {
        throw new Error(`Response status: ${response?.status}`);
      }
      ...
      const quantityMatch: QuantityObject | void = parseQuantity(

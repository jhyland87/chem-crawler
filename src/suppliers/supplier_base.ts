 ...
 */
export default abstract class SupplierBase<T extends Product>
  implements AsyncIterable<T>
{
  ...
  protected async httpGetHeaders(url: string): Promise<HeaderObject | void> {
    try {
      console.debug(
        "httpGetHeaders| this._controller.signal:",
      ...
      if (error instanceof Error && error.name === "AbortError") {
        ...
        console.error("Error received during fetch:", {
          error,
          signal: this._controller.signal,
        });
      }
  ...
  protected async httpGet(
    ...
    try {
      console.debug(
        "httpget| this._controller.signal:",
        this._controller.signal
      );
      return await fetch(url, {
        signal: this._controller.signal,
